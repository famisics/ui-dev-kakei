...
select u.id, 'サブスク', 'expense', '#ef4444', 0, true, 'subscription', p.id, array['CLAUDE','SPOTIFY','YOUTUBE','APPLE','アップル','GOOGLE','グーグル','CHATGPT','OPENAI','1PASSWORD','VERCEL','CURSOR','GITHUB','STEAM','HOYOVERSE','DISCORD','ADOBE','NETFLIX','CLOUDFLARE','TURSO','SURFSHARK','PADDLE','ITCH.IO','DMM','ディーエムエム','モリサワ','SQUARE ENIX','プライム','ネツトフリツクス','アベマ','ABEMA','UNCLUTTER','PIXIV','クリプトン','ABLETON AG']
from auth.users u
join public.categories p
...
select u.id, '趣味', 'expense', '#ef4444', 1, true, 'hobby', p.id, null
from auth.users u
join public.categories p
  on p.user_id = u.id and p.default_key = 'entertainment'
on conflict (user_id, default_key) where default_key is not null do update set
...
select u.id, '趣味(音楽)', 'expense', '#ef4444', 2, true, 'hobby_music', p.id, null
from auth.users u
join public.categories p
  on p.user_id = u.id and p.default_key = 'entertainment'
on conflict (user_id, default_key) where default_key is not null do update set
...
select u.id, '旅行', 'expense', '#ef4444', 3, true, 'travel', p.id, array['JAL','ジャル','ANA','エーエヌエー','ジェットスター','エアドゥ','ピーチアビエ','空港','クウコウ','航空','コウクウ','スカイショップ','ニュースター','NEWSTAR','スカイライナー','エキネット','ホテル','キングダム','AGODA','ミチノエキ','ブッサンカン','ヤツハシ','ウミホタル','北彩館','レンガテラス','オミヤゲ','ミソギノサト']
from auth.users u
join public.categories p
  on p.user_id = u.id and p.default_key = 'entertainment'
on conflict (user_id, default_key) where default_key is not null do update set
...
select u.id, 'イベント', 'expense', '#ef4444', 4, true, 'event', p.id, array['ｻﾞｲｺｶﾌﾞｼｷｶﾞｲｼｬ']
from auth.users u
join public.categories p
  on p.user_id = u.id and p.default_key = 'entertainment'
on conflict (user_id, default_key) where default_key is not null do update set
...
select u.id, '高額製品', 'expense', '#ef4444', 5, true, 'expensive_goods', p.id, null
from auth.users u
join public.categories p
  on p.user_id = u.id and p.default_key = 'entertainment'
on conflict (user_id, default_key) where default_key is not null do update set
...
select u.id, '立て替え', 'expense', '#eab308', 0, true, 'reimbursement_expense', p.id, null
from auth.users u
join public.categories p
  on p.user_id = u.id and p.default_key = 'other_expense'
on conflict (user_id, default_key) where default_key is not null do update set
  name = excluded.name,
  type = excluded.type,
  color = excluded.color,
...
select u.id, '払い戻し', 'expense', '#eab308', 1, true, 'refund', p.id, null
from auth.users u
join public.categories p
  on p.user_id = u.id and p.default_key = 'other_expense'
on conflict (user_id, default_key) where default_key is not null do update set
...
select u.id, '給与', 'income', '#3b82f6', 3, true, 'salary'
from auth.users u
on conflict (user_id, default_key) where default_key is not null do update set
  name = excluded.name,
  type = excluded.type,
...
select u.id, '仕送り', 'income', '#a855f7', 4, true, 'remittance'
from auth.users u
on conflict (user_id, default_key) where default_key is not null do update set
  name = excluded.name,
  type = excluded.type,
...
select u.id, '臨時収入', 'income', '#a855f7', 5, true, 'windfall'
from auth.users u
on conflict (user_id, default_key) where default_key is not null do update set
  name = excluded.name,
  type = excluded.type,
...
select u.id, 'その他', 'income', '#eab308', 6, true, 'other_income'
from auth.users u
on conflict (user_id, default_key) where default_key is not null do update set
  name = excluded.name,
  type = excluded.type,
...
select u.id, '立て替え', 'income', '#eab308', 0, true, 'reimbursement_income', p.id, null
from auth.users u
join public.categories p
  on p.user_id = u.id and p.default_key = 'other_income'
on conflict (user_id, default_key) where default_key is not null do update set
  name = excluded.name,
  type = excluded.type,
  color = excluded.color,
...
select u.id, '払い戻し', 'income', '#eab308', 1, true, 'refund_income', p.id, null
from auth.users u
join public.categories p
  on p.user_id = u.id and p.default_key = 'other_income'
on conflict (user_id, default_key) where default_key is not null do update set
...
declare
  ...
  cat_hobby uuid;
  cat_hobby_music uuid;
  cat_travel uuid;
  cat_event uuid;
  cat_expensive_goods uuid;
  cat_other_expense uuid;
  cat_reimbursement_expense uuid;
  cat_refund uuid;
  cat_salary uuid;
  cat_remittance uuid;
  cat_windfall uuid;
  cat_other_income uuid;
  cat_reimbursement_income uuid;
  cat_refund_income uuid;
begin
  insert into public.categories (
    user_id, name, type, color, sort_order, is_default, default_key, parent_id, import_keywords
  ) values (
  ...
  insert into public.categories (
    ...
    new.id, '趣味', 'expense', '#ef4444', 1, true, 'hobby', cat_entertainment, null
  ) returning id into cat_hobby;
  insert into public.categories (
    user_id, name, type, color, sort_order, is_default, default_key, parent_id, import_keywords
  ) values (
    new.id, '趣味(音楽)', 'expense', '#ef4444', 2, true, 'hobby_music', cat_entertainment, null
  ) returning id into cat_hobby_music;
  insert into public.categories (
    user_id, name, type, color, sort_order, is_default, default_key, parent_id, import_keywords
  ) values (
    new.id, '旅行', 'expense', '#ef4444', 3, true, 'travel', cat_entertainment, array['JAL','ジャル','ANA','エーエヌエー','ジェットスター','エアドゥ','ピーチアビエ','空港','クウコウ','航空','コウクウ','スカイショップ','ニュースター','NEWSTAR','スカイライナー','エキネット','ホテル','キングダム','AGODA','ミチノエキ','ブッサンカン','ヤツハシ','ウミホタル','北彩館','レンガテラス','オミヤゲ','ミソギノサト']
  ) returning id into cat_travel;
  insert into public.categories (
    user_id, name, type, color, sort_order, is_default, default_key, parent_id, import_keywords
  ) values (
    new.id, 'イベント', 'expense', '#ef4444', 4, true, 'event', cat_entertainment, array['ｻﾞｲｺｶﾌﾞｼｷｶﾞｲｼｬ']
  ) returning id into cat_event;
  insert into public.categories (
    user_id, name, type, color, sort_order, is_default, default_key, parent_id, import_keywords
  ) values (
    new.id, '高額製品', 'expense', '#ef4444', 5, true, 'expensive_goods', cat_entertainment, null
  ) returning id into cat_expensive_goods;
  insert into public.categories (
    user_id, name, type, color, sort_order, is_default, default_key, parent_id, import_keywords
  ) values (
    new.id, 'その他', 'expense', '#eab308', 2, true, 'other_expense', null, null
  ) returning id into cat_other_expense;
  insert into public.categories (
    user_id, name, type, color, sort_order, is_default, default_key, parent_id, import_keywords
  ) values (
    new.id, '立て替え', 'expense', '#eab308', 0, true, 'reimbursement_expense', cat_other_expense, null
  ) returning id into cat_reimbursement_expense;
  insert into public.categories (
    user_id, name, type, color, sort_order, is_default, default_key, parent_id, import_keywords
  ) values (
    new.id, '払い戻し', 'expense', '#eab308', 1, true, 'refund', cat_other_expense, null
  ) returning id into cat_refund;
  insert into public.categories (
    user_id, name, type, color, sort_order, is_default, default_key, parent_id, import_keywords
  ) values (
    new.id, '給与', 'income', '#3b82f6', 3, true, 'salary', null, null
  ) returning id into cat_salary;
  insert into public.categories (
    user_id, name, type, color, sort_order, is_default, default_key, parent_id, import_keywords
  ) values (
    new.id, '仕送り', 'income', '#a855f7', 4, true, 'remittance', null, null
  ) returning id into cat_remittance;
  insert into public.categories (
    user_id, name, type, color, sort_order, is_default, default_key, parent_id, import_keywords
  ) values (
    new.id, '臨時収入', 'income', '#a855f7', 5, true, 'windfall', null, null
  ) returning id into cat_windfall;
  insert into public.categories (
    user_id, name, type, color, sort_order, is_default, default_key, parent_id, import_keywords
  ) values (
    new.id, 'その他', 'income', '#eab308', 6, true, 'other_income', null, null
  ) returning id into cat_other_income;
  insert into public.categories (
    user_id, name, type, color, sort_order, is_default, default_key, parent_id, import_keywords
  ) values (
    new.id, '立て替え', 'income', '#eab308', 0, true, 'reimbursement_income', cat_other_income, null
  ) returning id into cat_reimbursement_income;
  insert into public.categories (
    user_id, name, type, color, sort_order, is_default, default_key, parent_id, import_keywords
  ) values (
    new.id, '払い戻し', 'income', '#eab308', 1, true, 'refund_income', cat_other_income, null
  ) returning id into cat_refund_income;
