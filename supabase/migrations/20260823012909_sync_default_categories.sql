...
select u.id, 'イベント', 'expense', '#ef4444', 2, true, 'event', p.id, array['ｻﾞｲｺｶﾌﾞｼｷｶﾞｲｼｬ']
from auth.users u
join public.categories p
  on p.user_id = u.id and p.default_key = 'entertainment'
on conflict (user_id, default_key) where default_key is not null do update set
...
select u.id, '高額製品', 'expense', '#ef4444', 3, true, 'expensive_goods', p.id, null
from auth.users u
join public.categories p
  on p.user_id = u.id and p.default_key = 'entertainment'
on conflict (user_id, default_key) where default_key is not null do update set
...
begin
  ...
  insert into public.categories (
    ...
    new.id, 'イベント', 'expense', '#ef4444', 2, true, 'event', cat_entertainment, array['ｻﾞｲｺｶﾌﾞｼｷｶﾞｲｼｬ']
  ) returning id into cat_event;
  insert into public.categories (
    user_id, name, type, color, sort_order, is_default, default_key, parent_id, import_keywords
  ) values (
    new.id, '高額製品', 'expense', '#ef4444', 3, true, 'expensive_goods', cat_entertainment, null
