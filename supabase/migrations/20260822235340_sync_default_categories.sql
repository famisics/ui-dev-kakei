...
select u.id, '給与', 'income', '#3b82f6', 0, true, 'salary'
from auth.users u
on conflict (user_id, default_key) where default_key is not null do update set
  name = excluded.name,
  type = excluded.type,
...
select u.id, '仕送り', 'income', '#a855f7', 1, true, 'remittance'
from auth.users u
on conflict (user_id, default_key) where default_key is not null do update set
  name = excluded.name,
  type = excluded.type,
...
select u.id, '臨時収入', 'income', '#a855f7', 2, true, 'windfall'
from auth.users u
on conflict (user_id, default_key) where default_key is not null do update set
  name = excluded.name,
  type = excluded.type,
...
select u.id, 'その他', 'income', '#eab308', 3, true, 'other_income'
from auth.users u
on conflict (user_id, default_key) where default_key is not null do update set
  name = excluded.name,
  type = excluded.type,
...
begin
  ...
  insert into public.categories (
    ...
    new.id, '給与', 'income', '#3b82f6', 0, true, 'salary', null, null
  ) returning id into cat_salary;
  insert into public.categories (
    user_id, name, type, color, sort_order, is_default, default_key, parent_id, import_keywords
  ) values (
    new.id, '仕送り', 'income', '#a855f7', 1, true, 'remittance', null, null
  ) returning id into cat_remittance;
  insert into public.categories (
    user_id, name, type, color, sort_order, is_default, default_key, parent_id, import_keywords
  ) values (
    new.id, '臨時収入', 'income', '#a855f7', 2, true, 'windfall', null, null
  ) returning id into cat_windfall;
  insert into public.categories (
    user_id, name, type, color, sort_order, is_default, default_key, parent_id, import_keywords
  ) values (
    new.id, 'その他', 'income', '#eab308', 3, true, 'other_income', null, null
